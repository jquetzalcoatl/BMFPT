...
function firstPassage(Q)
    ...
    for i=1:limit
        ...
        if tt<tau2
            ...
            #println(T)
            #T=T+tau2
            newposition= mod(i,2)==0 ?  10 : 15       #rand(Normal(0,sqrt(2*T)))       # T==0 ? 0 : rand(Levy(0,sqrt(2*T)))            #rand(Normal(0,sqrt(2*T)))       #New position random variable
            T=T+tau2
        end
        if i==limit
            println("increase limit")
...
plot(QQ[1:13]', [i for i in meantime]', seriestype=:scatter, xaxis=(:log),yaxis=(:log))

println([QQ' meantime]')

writedlm("FPTv2.dat",[QQ[1:14] meantime]')
